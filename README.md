...
- [ ] **뉴스 목록 불러오기**: `GET /api/news` 연동 및 메인 화면 리스트 렌더링
- [ ] **뉴스 상세 보기**: `GET /api/news/{id}` 연동 및 상세 페이지 데이터 바인딩
- [ ] **상태 관리 및 에러 처리**: 로딩 중(Spinner) UI 및 데이터 로드 실패 시 에러 메시지 처리

### Phase 3: AI 분석 기능 및 부가 기능 구현 (AI & Advanced Features)
- [ ] **AI 분석 요청 기능**: 상세 페이지 내 'AI 분석하기' 버튼 및 `POST /api/analyze` 연동
- [ ] **분석 결과 UI 시각화**: 
  - 신뢰도 점수 게이지 바 형태로 구현
  - 어려운 용어 / 핵심 인물 팝업 또는 툴팁 처리
- [ ] **뉴스 검색 기능**: Header 검색창과 `GET /api/search` 연동
- [ ] **수동 크롤링 트리거**: 관리자용 `POST /api/crawl` 버튼 구현 (데모 시연용)
